...
class ScriptEngine:
    ...
    def find_image_location(self, image_path):
        if not self.running: return None # More responsive stop
        try:
    ...
    def run_script(self, script):
        ...
        while i < len(lines) and self.running:
            # FIX: Strip leading/trailing whitespace to allow indentation
            line = lines[i].strip()
            current_line_num = i
            i += 1 
            # FIX: Ignore comments and empty lines
            ...
            try:
                ...
                if command.startswith('if_'):
                    if not self.handle_if(command, args): i = self.find_matching_block_end(lines, i, 'if_', 'endif')
                elif command == 'loop':
                    ...
                    else: self.update_output(f"Error: 'endloop' without a matching 'loop' on line {current_line_num + 1}.")
                elif command == 'break':
                ...
                # FIX: Correctly recognize 'endif' as a valid (but passive) command
                elif command == 'endif':
                    # This is handled by the `find_matching_block_end` logic.
                    # If we encounter it here, it's likely an extra, misplaced endif.
                    self.update_output(f"Warning: Misplaced 'endif' on line {current_line_num + 1}. Ignoring.")
                else:
                    handler = getattr(self, f"handle_{command}", None)
                    if handler:
                        if not self.running: break # Check before every command
                        handler(args)
                    else:
        ...
        status = "Finished" if self.running else "Stopped"
        self.update_status(status)
        self.running = False

    def stop_script(self):
        if self.running: self.running = False; self.update_output("Stop signal received. Finishing current action...")

    def perform_mouse_action(self, action_func, location, action_name):
        if not self.running: return # More responsive stop
        if not location: self.update_output(f"Action '{action_name}' failed: target not found."); self.running = False; return
    ...
    def handle_wait(self, args):
        seconds = float(args);
        # FIX: Make wait interruptible by F6
    ...
    def handle_select_window(self, args):
        title = args.strip('"');
        try: pyautogui.getWindowsWithTitle(title)[0].activate(); self.update_output(f"Activated window: {title}")
    ...
    def handle_eval(self, args):
        var_name, expression = [p.strip() for p in args.split('=', 1)]; self.variables[var_name] = self._evaluate_expression(expression)
    ...
    def handle_script(self, args):
        ...
        try:
            ...
            sub_engine.run_script(script_content)
            if not sub_engine.running and self.running: self.running = False
            self.variables = sub_engine.variables
    ...
    def handle_if(self, command, args):
        if not self.running: return False # More responsive stop
        if command == "if_eval": return self._evaluate_expression(args)
        if command == "if_image_screen": return self.find_image_location(args.strip('"')) is not None
        if command == "if_text_screen": return self.find_text_location(args.strip('"')) is not None
        if command == "if_text_region":
    ...
    def find_matching_block_end(self, lines, start_index, start_cmd_prefix, end_cmd):
        level = 1; i = start_index
        while i < len(lines):
            line = lines[i].strip()
            # Handle nested blocks of the same type
            if line.startswith(start_cmd_prefix): level += 1
            elif line == end_cmd: level -= 1
            if level == 0: return i + 1
            i += 1
        self.update_output(f"Error: Missing '{end_cmd}' for block starting near line {start_index}."); self.running = False; return len(lines)